...
# ---------------------------
# Persistent Popup variables
# ---------------------------
popup_thread = None
popup_shown = False
popup_display_flag = None
popup_message = None
popup_message_lock = threading.Lock()

# For managing popup message duration
popup_message_start_time = 0
POPUP_MIN_DISPLAY_TIME = 5.0  # seconds minimum to show popup


def intruder_popup_thread_fn(display_flag: threading.Event):
    root = tk.Tk()
    root.title("Alert")
    root.attributes("-topmost", True)
    root.resizable(False, False)

    label = tk.Label(root, font=("Helvetica", 28, "bold"), fg="white", bg="red", padx=30, pady=20)
    label.pack()
    ...
    def check_display():
        nonlocal label
        with popup_message_lock:
            current_msg = popup_message

        if display_flag.is_set() and current_msg:
            # Check minimum display duration
            global popup_message_start_time
            elapsed = time.time() - popup_message_start_time
            if elapsed < POPUP_MIN_DISPLAY_TIME:
                root.deiconify()
                label.config(text=current_msg)
            else:
                # Hide popup after minimum time if display_flag cleared
                if not display_flag.is_set():
                    root.withdraw()
            # Continue showing popup if flag still set
            root.deiconify()
            label.config(text=current_msg)
        else:
            root.withdraw()

        root.after(100, check_display)
...
def show_popup(message):
    global popup_thread, popup_shown, popup_display_flag, popup_message, popup_message_start_time
    with popup_message_lock:
        # Set new message and start time
        if popup_message != message:
            popup_message = message
            popup_message_start_time = time.time()
        else:
            # If same message, keep start time if still showing
            if popup_message_start_time == 0:
                popup_message_start_time = time.time()
    if not popup_shown:
        popup_display_flag = threading.Event()
        popup_thread = threading.Thread(target=intruder_popup_thread_fn,
                                        args=(popup_display_flag,), daemon=True)
        popup_thread.start()
        popup_shown = True
    popup_display_flag.set()


def hide_popup():
    global popup_display_flag, popup_message_start_time
    # Delay clearing popup only after minimum display time
    if popup_shown and popup_display_flag:
        elapsed = time.time() - popup_message_start_time
        if elapsed >= POPUP_MIN_DISPLAY_TIME:
            popup_display_flag.clear()
            with popup_message_lock:
...
def detect_emotion(face_landmarks):
    lm = face_landmarks
    # Landmark indices as per mediapipe face mesh standard
    TOP_LIP = 13
    BOTTOM_LIP = 14
    LEFT_LIP = 61
    RIGHT_LIP = 291
...
def detect_posture(p_landmarks, w, h):
    ...
    wrists_above_shoulders = (lw_y < ls_y) and (rw_y < rs_y)
    wrists_close_enough = wrist_dist < 0.45 * shoulder_width
    if wrists_close_enough and wrists_above_shoulders:
    ...
    # Leaning forward/back detection fixes:
    # Positive z_diff means shoulders in front of nose (lean forward)
    # Negative z_diff means shoulders behind nose i.e. leaning back
    # Use threshold with small margin for robustness
    if z_diff > 0.04:
...
def detect_gestures(p_landmarks, w, h):
    ...
    results = {"Hand Raised": False, "Pointing": False}
...
def main():
    ...
    prev_time = time.time()
    fps = 0.0
    global behavior_counts, emotion_label
        ...
        while cap.isOpened():
            ...
            if pose_results.pose_landmarks:
                ...
                if left_object or right_object:
                    ...
                    show_popup("Kya hila rahe ho?")  # Show popup with 5s minimum duration
